...
def run_migration(srv_config, query_funcs, timeout_limit=300):
    ...
    if _available:
        driver = "{" + sorted(_available, reverse=True)[0] + "}"
    else:
        driver = "{ODBC Driver 18 for SQL Server}"  # fallback
    start_time = time.time()
    global has_error
    
    conn_str = (
        f"DRIVER={driver};SERVER={srv_config['ip']};DATABASE={srv_config['db']};"
        f"UID={srv_config['user']};PWD={srv_config['pw']};Connection Timeout=30;TrustServerCertificate=yes;"
    )
